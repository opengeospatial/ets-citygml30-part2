...
public class GlobalValidation extends CommonFixture {
	...
	/**
	 * Verify that XML instance documents claiming conformance to this specification
	 * validate against the XML schema files.
	 * @throws Exception
	 */
	...
	/**
	 *
	 * Verify that XLinks are not used to reference geometries, except for geometries of
	 * ImplicitGeometry elements, from another top-level feature.
	 * @throws Exception
	 */
	@Test(enabled = GLOBAL_ENABLE)
	 ...
	 * <ul>
	 * <li>Geometries stored inline a space boundary are not redundantly stored as
	 * geometry of a space. If the geometry is stored with the space in addition, it
	 * references the geometry from the space boundary using XLinks.
	 *
	 * <li>Space boundaries do not reference geometries of a space using XLinks.
	 *
	 * <li>This applies for all spaces and space boundaries that are children of the same
	 * top-level feature.
	 * </ul>
	 */
	@Test(enabled = GLOBAL_ENABLE)
	public void VerifyGlobalReferencingGeometries2() throws XPathExpressionException {
		...
		try {
			...
			for (String spaceBoundaryName : spaceBoundaryNamesList) {
				...
				NodeList spaceBoundariesNode = (NodeList) xPath.evaluate(expr, this.testSubject,
	...
	/**
	 * Verify that LoDs are self-contained: Geometries are not shared between different
	 * LoDs using XLinks.
	 */
	@Test(enabled = GLOBAL_ENABLE)
	...
	/**
	 * When referencing shared geometries between top-level features using
	 * CityObjectRelation elements, verify that:
	 * <ul>
	 * <li>If two top-level features share a common geometry, the shared geometry is
	 * stored for each top-level feature separately.
	 *
	 * <li>If CityObjectRelation elements are used to denote that a geometry is shared
	 * between two top-level features, then a CityObjectRelation element is stored for
	 * each feature (might be the top-level feature itself or one of its nested features),
	 * and each CityObjectRelation element is assigned the relation type “shared” and
	 * references the other feature using an XLink. Thus, the reference is bi-directional.
	 * </ul>
	 */
	@Test(enabled = GLOBAL_ENABLE)
	public void VerifyGlobalReferencingGeometries4() {
		List<String> topLevelFeature = new ArrayList<>(Arrays.asList("Bridge", "Building", "CityFurniture",
				"CityObjectGroup", "GenericLogicalSpace", "GenericOccupiedSpace", "GenericThematicSurface",
				"GenericUnoccupiedSpace", "LandUse", "OtherConstruction", "PlantCover", "Railway", "ReliefFeature",
				"Road", "SolitaryVegetationObject", "Square", "Track", "Tunnel", "WaterBody", "Waterway"));
		...
		for (int i = 0; i < nodeList.getLength() && topLevelSeparately; i++) {
			...
			if (!topLevelFeature.contains(localName))
				continue;
			String relatedId = XMLUtils
				.getNodeByXPath(n, "core:relatedTo/core:CityObjectRelation/core:relatedTo/@xlink:href")
				.getNodeValue();
		...
		Assert.assertTrue(topLevelSeparately,
				"The shared geometry is NOT stored for each top-level feature separately.");
	 ...
	 * <ul>
	 * <li>Each feature belongs to a natural aggregation hierarchy and SHALL be stored
	 * inline this hierarchy.
	 *
	 * <li>Alternative aggregations SHALL NOT contain the feature inline but SHALL use an
	 * XLink to reference the feature.
	 * </ul>
	 */
	@Test(enabled = GLOBAL_ENABLE)
	...
	private void checkXLink(Node node) {
		try {
			...
			Assert.assertTrue(nodeList.getLength() > 0,
					String.format("Element '%s' contains non resolvable xlink '%s'.", nodeName, xlinkHrefTextContent));
		}
		catch (Exception e) {
